...
```{r}
mort$res_status <- as.numeric(mort$res_status)
mort <- mort %>%
  filter(!is.na(res_status)) %>%
  mutate(res_status = case_when(res_status == 1 ~ "resident",
                                res_status == 2 ~ "intrastate nonresident",
                                res_status == 3 ~ "interstate nonresident",
                                res_status == 4 ~ "foreign resident"))
```

We do the same for education. Running this initally returned a very large number of missing values for education, so I want to verify that this occurred in the set that has not modified `education` at all.

```{r}
edu_missing <- count(mort %>% filter(is.na(education)))
edu_avail <- count(mort %>% filter(!is.na(education)))
print(edu_missing)
print(edu_avail)
print(edu_missing/nrow(mort)*100)
```

We see that almost 17% of cases are missing values for education. This was not as high a proportion as I had worried when initially running the following code, but it is still something to examine. This will be visited later.

```{r}
mort$education <- as.numeric(mort$education)
mort <- mort %>%
  filter(!is.na(education)) %>%
  mutate(education = case_when(education == 00 ~ "No formal education",
                               education == 1:8 ~ "Elementary school",
                               education == 9 ~ "1 year of high school",
                               education == 10 ~ "2 years of high school",
                               education == 11 ~ "3 years of high school",
                               education == 12 ~ "4 years of high school",
                               education == 13 ~ "1 year of college",
                               education == 14 ~ "2 years of college",
                               education == 15 ~ "3 years of, college",
                               education == 16 ~ "4 years of college",
                               education == 17 ~ "5 or more years of college",
