...
```{r}
write.table(pop_table, file="demographic_table.csv", sep=",", row.names=FALSE)
```
...
pop_table <- pop_table[69:85,]
pop_table <- rename(pop_table, average_population="Average population[67][29][30]")
pop_table$year <- as.numeric(pop_table$year)
pop_table$average_population <- as.numeric(gsub(",","", pop_table$average_population))
```