...
```{r}
cardio_pre_locs <- cardio_pre %>%
  group_by(death_loc_decoded) %>%
  summarise(locs = n(),
            percent = n() / nrow(cardio_pre) * 100) %>%
  arrange(desc(percent))

cardio_post_locs <- cardio_post %>%
  group_by(death_loc_decoded) %>%
  summarise(locs = n(),
            percent = n() / nrow(cardio_post) * 100) %>%
  arrange(desc(percent))
```
...
```{r}
cardio_pre_race <- cardio_pre %>%
  group_by(race_decoded) %>%
  summarise(count = n(),
            percent = n() / nrow(cardio_pre) * 100) %>%
  arrange(desc(percent))

cardio_post_race <- cardio_post %>%
  group_by(race_decoded) %>%
  summarise(count = n(),
            percent = n() / nrow(cardio_post) * 100) %>%
  arrange(desc(percent))
...
Many fewer races are represented in the pre-2018 data; making sure that there isn't something odd about the data by looking at all pre-2018 races.
